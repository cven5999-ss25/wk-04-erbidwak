...
composition_tidy <- composition %>%
  ...
  rename_with(
    .fn = ~ gsub("^composition_|_percent$", "", .x),
...
```{r}
composition_tidy |>
  filter(income_id %in% c("HIC", "LIC"), other > 40)
```
...
composition_tidy_capita <- composition_tidy %>%
  mutate(msw_capita_kg_year = (msw_tons_year * 1e6) / population) %>%  # Convert tons to kg
...
<!--# It renames the short name to fullname if the income_id is equal to the given short name -->

3.  How is the object `levels_income_cat` used and what does it achieve?

<!--# levels_icome_cat is used after the case_when has executed, and is it becomes the values of income_cat -->
...
<!--# The dataset is grouped by income_id with the number of occurence of each -->
...
<!--# It does the same, but now counting the income_cat -->
...
5.  Execute the code and observe the output in the Console. What has changed? How is that different to adding `na.rm = TRUE` as an argument to the `mean()` ?
    ...
    ***NA has gone. I think na.rm does the same, i can expect any difference***
...
table_composition <- composition_tidy_capita_income_cat |>
  filter(
    !is.na(msw_capita_kg_year),
    !is.na(food_organic_waste),
    !is.na(plastic),
    !is.na(paper_cardboard)
  ) |>
  group_by(income_cat) |>
  summarise(
    count = n(),
